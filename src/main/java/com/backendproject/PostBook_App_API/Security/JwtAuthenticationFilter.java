...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        // GET TOKEN in the form of (Bearer something) , where something is token
        String requestToken = request.getHeader("Authorization");    // It's Key for token,as token will come from header. So, we will send our key in header
        ...
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if (this.jwtTokenHelper.validateToken(token, userDetails)) {
                ...
                request.setAttribute("loginUser",userDetails);  //**Adding this Extra Line, Here we are setting Attributes of user(logged-In) from UserDetails using Token of logged-In user.
                                                                   // So,that on doing comment his details are on display. We are using this "loginUser" as a modelAttribute in comment Controller**//
